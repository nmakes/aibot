...
class CG:
	...
	def constructCG(self, packages):
		...
		for var in self.nodes: # for every variable, restrict the domain (apply unary constraints)
			...
			(courseName, component, rank) = var.split(' ')
			...
			"""
			if (courseName in self.csp.professorList['Prof-04']) & (component=='L'):
				dom = self.domains[var]
				self.domains[var] = [v for v in dom if v[0]!='thu']

			if (courseName in self.csp.professorList['Prof-01']) & (component=='L'):
				dom = self.domains[var]
...
def displayTabularOutput(TableHead, TableData):
	print "-----------------------------------------------------------------"
	print "%-8s | %-10s | %-10s | %-8s | %-10s | %-6s" % (TableHead[0], TableHead[1], TableHead[2], TableHead[3], TableHead[4], TableHead[5])
	print "-----------------------------------------------------------------"
	for data in TableData:
		print "%-8s | %-10s | %-10s | %-8s | %-10s | %-6s" % (data[0], data[1], data[2], data[3], data[4], data[5])